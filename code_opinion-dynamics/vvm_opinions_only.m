...
for k = 1:M-1
    ...
    Psi_ll = psi(wl(:,k),wl(:,k),Nl,Nl,w_r);
    Psi_ff = psi(wf(:,k),wf(:,k),Nf,Nf,w_r);
    Psi_uu = psi(wu(:,k),wu(:,k),Nu,Nu,w_r);
    
    %% Leaders
    % Change opinion after interaction with followers
    Psi_lf = psi(wl(:,k),wf(:,k),Nl,Nf,w_r);

    % Don't change opinion after interaction with followers
    % Psi_lf = psi(wl(:,k),wf(:,k),Nl,Nf,0.1);

    % Change opinion after interaction with uninformed
    Psi_lu = psi(wl(:,k),wu(:,k),Nl,Nu,w_r);
    ...
    Psi_fl = psi(wf(:,k),wl(:,k),Nf,Nl,w_r);

    % Change opinion after interaction with uninformed
    Psi_fu = psi(wf(:,k),wu(:,k),Nf,Nu,w_r);

    % Don't change opinion after interaction with uninformed
    % Psi_fu = psi(wf(:,k),wu(:,k),Nu,Nu,w_r);    

    %% Uninformed
    % Always change opinion 
    Psi_ul = psi(wu(:,k),wl(:,k),Nu,Nl,w_r);
    Psi_uf = psi(wu(:,k),wf(:,k),Nu,Nf,w_r);

    % Remained uninformed 
    % Psi_ul = psi(wu(:,k),wl(:,k),Nu,Nl,w_r);
    % Psi_uf = psi(wu(:,k),wf(:,k),Nu,Nu,w_r);
