...
% Problem data
NL = 100;                             % number of individuals from each group
%Ca = 1; la = 1; Cr = 0.5; lr = 0.5; % attraction-repulsion parameters
Ca = 1; la = 1; Cr = 0.5; lr = 1.2;
T = 60;                             % final time
dt = 1.0e-2;                        % timestep
...
alpha_norm = alpha * beta * la^2;
m_norm = 1/(beta^2 * Ca * la^2);
C = Cr/Ca;
L = lr/la;
...
w(:,1) = w0;
[F1x, F1w] = F(x(:,:,1), w(:,1), NL, alpha_norm, m_norm, C, L, rx, rw, taur, taub); % Initial F value

% Use Euler for the first step
x(:,:,2) = x(:,:,1) + dt * F1x; 
w(:,2) = w(:,1) + dt * F1w; 
% Compute F for the second step (needed for Adams-Bashforth)
[F2x, F2w] = F(x(:,:,2), w(:,2), NL, alpha_norm, m_norm, C, L, rx, rw, taur, taub);
...
for i = 3:M
    ...
    F1w = F2w;
    [F2x, F2w] = F(x(:,:,i), w(:,i), NL, alpha_norm, m_norm, C, L, rx, rw, taur, taub);  % F for current step
...
hold on;
for i = 1:N
   plot(squeeze(x(i, 1, 5196:5999)),  squeeze(x(i, 2, 5196:5999)), 'r'); 
end
hold on
...
function [Fx,Fw] = F(x, w, NL, alpha_norm, m_norm, C, L, rx, rw, taur, taub)
    N = NL;
    l = x(1:NL,:);
    v = x(N+1:(N+NL),:);
    wu = w;
    Fx = [v; ...
          (1/N)*(1/m_norm)*sumU(l, NL, C, L) + (alpha_norm - sum(v.^2,2)).*v*(1/N) ...
...
function sumU = sumU(x, N1, C, L)
    sumU = zeros(size(x, 1), size(x, 2)); 
    for i = 1:size(sumU, 1)
        for j = 1:N1
            if j ~= i
                sumU(i,:) = sumU(i,:) - morsepotential(sqrt(sum((x(i,:)-x(j,:)).^2,2)), C, L) .* (x(i,:) - x(j,:)) ./ norm(x(i,:)-x(j,:),2);
            end
        end
    end
end

function morsepotential = morsepotential(r, C, L)
    morsepotential = -exp(-r) + C*exp(-r./L);
end
