...
for i = 2:Nt
    ...
    velocity = velocity + prefac3 * permute(sum(morseUU, 2), ndims(sum(morseUU, 2)):-1:1) + prefac1 *  permute(sum(morseUL, 2), ndims(sum(morseUL, 2)):-1:1) + prefac2 * permute(sum(morseUF, 2), ndims(sum(morseUF, 2)):-1:1) ...
